...
let project = Project(
	...
	targets: [
		.target(
			...
			//							debug: .app_debug.merging(.secret_debug).merging([
			//								"PROVISIONING_PROFILE_SPECIFIER": "match Development com.iyc.notifyme",
			//							]),
			//							release: .app_release.merging(.secret_release).merging([
			//								"PROVISIONING_PROFILE_SPECIFIER": "match AppStore com.iyc.notifyme",
			//							])
			//						)
			settings: .settings(
				base: .app_base.merging(.secret_base),
				...
				release: .app_base.merging(.app_release).merging(.secret_release).merging([
					"PROVISIONING_PROFILE_SPECIFIER": "TO"
				])
			),
		),
		//		.target(
